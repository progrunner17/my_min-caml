...
let rec deref_typ = function (* 型変数を中身でおきかえる関数 (caml2html: typing_deref) *)
  ...
  | Type.Var({ contents = Some(t) } as r) ->
      ...
      r := Some(t');
      t'
(* | Unit| Bool| Int| Float  が来たらそのまま返す
Typing.g で
    | Bool(_) -> Type.Bool
  みたいな形で生成される
*)
...
let rec deref_term env = function
  ...
  | Add(e1, e2) -> (
        ...
        let et2 = g env e2 in
        match et1,et2 with
        | Type.Float,Type.Float -> FAdd(deref_term env e1, deref_term env e2)
        | _,_ -> Add(deref_term env e1, deref_term env e2))
  | Sub(e1, e2) -> (
        let et1 = g env e1 in
        let et2 = g env e2 in
        match et1,et2 with
        | Type.Float,Type.Float -> FSub(deref_term env e1, deref_term env e2)
        | _,_ -> Sub(deref_term env e1, deref_term env e2))
  ...
  (* deref_id_typ がメインの動作 *)
  | Let(xt, e1, e2) -> Let(deref_id_typ xt, deref_term env e1, deref_term env e2)
...
(* Int(1)みたいに来た次は1が来るので1を返すみたいな感じ *)
  | e -> e


and occur r1 = function (* occur check (caml2html: typing_occur) *)
  | Type.Fun(t2s, t2) -> List.exists (occur r1) t2s || occur r1 t2
  | Type.Tuple(t2s) -> List.exists (occur r1) t2s
  | Type.Array(t2) -> occur r1 t2
  | Type.Var(r2) when r1 == r2 -> true
  (* 参照の中身を調べたい時に{contents = } という形をつくる *)
...
(* Type.t -> () ただし、失敗したらerror を raiseする *)
and unify t1 t2 = (* 型が合うように、型変数への代入をする (caml2html: typing_unify) *)
  match t1, t2 with
...
and g env e = (* 型推論ルーチン (caml2html: typing_g) *)
  try
    match e with
    | Unit -> Type.Unit
    | Bool(_) -> Type.Bool
...
(* int で型推論に失敗したら floatだと思って推論*)
    ...
    | Put(e1, e2, e3) ->
        ...
        unify Type.Int (g env e2);
        Type.Unit
  with Unify(t1, t2) -> raise (Error(deref_term env e, deref_typ t1, deref_typ t2))
...
*)
  (try unify Type.Unit (g M.empty e) (*() と eを 推論した型をunify  topは()になるはずだから？*)
